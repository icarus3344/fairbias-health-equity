...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run on small subsample (500 rows) for fast smoke verification.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="runs/d8_enhancement_study",
        help="Output directory for study results and manifest.",
    )
    ...
    for arm_id, arm_data in results.items():
        ...
        for cond_name, cond_res in conds.items():
            ...
            rows.append({
                ...
                "condition": cond_name,
                "num_transforms": cond_res["num_transforms"],
                "test_auroc": round(test_res["auroc"], 4),
                "test_auprc": round(test_res["auprc"], 4),
...
def main() -> None:
    ...
    runner = D8EnhancementRunner(smoke_test=args.smoke_test)
    ...
    # Build and save comparative table
    ...
    print("\n### Downstream Test (2024) Utility and Fairness Summary:\n")
    cols_to_display = [
        "arm_id",
        "condition",
        "test_auroc",
