...
def main() -> None:
    ...
    if args.output_dir is not None:
        ...
        if out_dir.exists() and (
            (out_dir / "d8_enhancement_study_results.json").exists()
            or (out_dir / "d8_enhancement_comparison.csv").exists()
        ):
            raise FileExistsError(
                f"Output directory {out_dir} already exists and contains previous study results. "
