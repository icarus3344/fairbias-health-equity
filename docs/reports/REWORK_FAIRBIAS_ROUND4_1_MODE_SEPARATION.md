...
Status: COMPLETE — golden fixture 已纳入受控提交范围；`paper_strict` 命名废除，代之以两个显式定义的算法模式 `official_unweighted_reproduction`（MDS 固定 dim=2、官方交错幂流、无迭代预算、无 Pareto 回退）与 `engineering_bounded`（自动维数、六值网格、有限预算、Pareto 扩展）；survey-weighted 扩展以可测试声明钉住；提交边界审计文档补录（Decision 0005）；全量套件 237/237 OK；双模式实证重跑完成（official 模式下 Credit 首次进入 ε 球收敛）。clean-checkout 验证基于暂存树（`git write-tree` + `git archive`）通过后方才提交。
...
| P1-2 | `paper_strict` 名不副实：自动 dim=3 vs 官方固定 dim=2；六值网格 vs 官方交错流 `[3,1/3,…,1999,1/1999]`；有限预算 | **模式架构**（[src/fairbias/config.py](../../src/fairbias/config.py)）：新增 `algorithm_mode` ∈ {`official_unweighted_reproduction`, `engineering_bounded`}（构造期校验）与 `resolved()` 单点解析；`official_power_stream()` 程序化生成官方交错流（与官方 L198 `np.array([[i,1/i] for i in range(3,2000,2)]).reshape(-1)` 逐元素一致，含顺序）；`mds_fixed_components` 支持固定维数短路（official 强制 2，非 2 显式拒绝）。official 语义：MDS dim=2、官方幂流**保持交错顺序**（[mitigation.py](../../src/fairbias/mitigation.py) 新增 `preserve_exponent_order`，官方顺序 ≠ 升序排序——升序 (1/7,1/5,1/3,3,5,7) 与交错 (3,1/3,5,1/5,…) 是实质差异）、**无有限迭代预算**（循环仅由 ε 球或幂流穷尽终止；官方幂流有限且每属性幂次严格推进 ⇒ 终止有保证）、无 Pareto 回退（输出仅 `final_results_official_unweighted_reproduction` 单终态键）；同时拒绝 accuracy enhancement 与 failed_attribute_mode='next'（均为具名工程扩展）。engineering 语义 = 原 round-4 行为诚实命名：贪心终态键更名 `final_results_configured_greedy_terminal`（`paper_strict` 键与 `FairBiasRunResult.paper_strict_*` 字段全部移除），Pareto 键保留 | `tests/test_fairbias_algorithm_modes.py`（19 项新测试）：幂流内容/顺序/长度/尾部、官方构造直译对照、resolved 各拒绝分支、固定维数跳过肘部（mock 断言 `_find_optimal_mds_components` 未被调用）、official 单终态且 payload 无任何 pareto 键、`max_iterations=1` 在 official 被忽略（终止原因必非 `iteration_budget_exhausted`）而同参数 engineering 必触发预算耗尽、顺序保留 vs 升序排序 |
...
**COMPAS official_unweighted_reproduction**（ε=0.0020，运行 0.7s）：
...
**Credit official_unweighted_reproduction**（ε=0.0002，运行 5.5s）——**本轮关键观察**：
...
**表述边界**：以上全部为单 seed、单划分下的描述性观测。official 模式复现的是**方法**（公式 + 贪心控制流 + 官方实现裁决细节），不复现论文实验协议（Optuna 超参优化、validation early stopping、中型数据 10 次 test 汇总）——按监督者"方法复现严格"的定义执行。

## 4. 标准报告模板

Gate: REWORK round 4.1（如上）

Status: COMPLETE（§1 四项判定按范围裁定全部落地；237/237 测试通过；双模式实证完成；clean-checkout 验证基于暂存树通过后提交）
...
Commands executed:
- `PYTHONPATH=src .venv311/bin/python -m unittest discover -s tests -p "test_*.py" -v`（全量：`Ran 237 tests in 40.198s` / `OK`）
...
Tests executed: 全量套件（237 项）+ fairbias 定向（82 项）+ clean-checkout 暂存树全量套件。

Exact test results:
- 工作区全量：`Ran 237 tests in 40.198s` / `OK`（round-4 为 217 项，净增 20：19 个模式契约测试 + 1 个 fixture 跟踪防护测试）
...
Assumptions:
1. official 模式复现的边界 = 论文公式 + 贪心控制流 + 官方实现作为正文含糊处的可执行裁决依据（Data Availability 指定仓库）；不复现论文实验协议（分类器超参、Optuna、多 seed、10 次 test 汇总）。
2. 官方幂流上限 1999 来自官方代码常量（`range(3,2000,2)`），不揣测论文正文含糊处以外的语义。
3. official 模式迭代无预算的终止保证：幂流有限（1998 项）且每个被接受的变换严格推进该属性的幂次或类别合并链，类别合并链受类别数上界约束；ε 球内不接受任何变换（沿袭 round-3/4 语义）。
