#!/usr/bin/env python3
"""Raise CPU runtime shard throughput after draining the original dispatcher.
...
def await_released_run_lock(run: Path, timeout: float = 10.0):
    """A killed process's visible zombie state can precede final lock release."""
...
def _safe_stop_at_boundary(pid: int, run: Path, runner: Path):
    ...
    prior_shard = str(ROOT / "scripts/run_nhis_cpu_shard.py").encode()
    prior_runtime = str(ROOT / "scripts/run_nhis_runtime_shard.py").encode()
    valid_parent = ((prior_runtime in args and b"--role" in args and b"cpu" in args) or
                    (normal in args and b"develop" in args) or
                    (handoff in args and b"--run" in args) or
                    (prior_shard in args and b"--run" in args and b"--role" in args and b"source" in args))
...
class ShardedScheduler(ParallelBenchmarkScheduler):
    ...
    def _validate_plan(self) -> None:
        ...
        if 'runtime_variant' in plan:
            ...
            required = {'scripts/run_nhis_runtime_worker.py',
            ...
            if set(files) != required:
                raise SchedulerError('incomplete runtime source manifest')
            for relative, expected in files.items():
                if not (self.root / relative).is_file() or _sha(self.root / relative) != expected:
                    raise SchedulerError(f'runtime source changed: {relative}')
            if identity({'variant': plan['runtime_variant'], 'files': files,
                         'environment': plan.get('runtime_environment')}) != plan.get('runtime_source_identity'):
                raise SchedulerError('runtime source identity mismatch')
    ...
    def _runtime_environment(self, spec):
        if spec.config['method'] != 'FRAPPE_EO' or 'runtime_variant' not in self.plan:
            return {}
        if self.plan['runtime_variant'] != 'frappe_pipeline_deterministic_v1_20260917':
            raise SchedulerError('unknown runtime recovery variant')
        environment = self.plan.get('runtime_environment')
        if environment != {'TF_DETERMINISTIC_OPS': '1'}:
            raise SchedulerError('runtime recovery must match the tested environment')
        return dict(environment)

    def _environment(self, spec):
        environment = super()._environment(spec)
        environment.update(self._runtime_environment(spec))
        return environment

    def _job_payload(self, spec):
        payload = super()._job_payload(spec)
        environment = self._runtime_environment(spec)
        if environment:
            payload['runtime_variant'] = self.plan['runtime_variant']
            payload['runtime_environment'] = environment
            payload['runtime_plan_sha256'] = _sha(self.plan_path)
            payload['runtime_source_files'] = self.plan['runtime_source_files']
            payload['runtime_source_identity'] = self.plan['runtime_source_identity']
        return payload
    ...
    def _write_shard_audit(self) -> None:
        payload = {
            ...
            "admission": ({"enabled": self.role == "cpu", "floor_gib": 1.5,
                           "aggregate_reserve_gib": 1.0, "host_reserve_gib": 6.0,
                           "observed_peak_multiplier": 1.1,
                           "max_workers": self.limits.workers} if self.role == "cpu"
                          else {"enabled": False}),
        }
        target = self.run_path / f"shard_{self.role}_capacity_activation.json"
        if target.exists():
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--role", required=True, choices=("cpu",))
    ...
    try:
        ...
        runner_name = 'run_nhis_runtime_worker.py' if 'runtime_variant' in plan else 'run_nhis_benchmark.py'
        scheduler = ShardedScheduler(args.run, runner=ROOT / 'scripts' / runner_name,
        ...
        if args.old_pid is not None:
            if args.role != "cpu":
                raise SchedulerError("capacity handoff is CPU-only")
            import datetime as dt
            import handoff_nhis_parallel as handoff
            from nhis_fairbias.benchmark.parallel_execution import ExclusiveRunLock
            audit = scheduler.run_path / "operational_handoffs" / (
                dt.datetime.now(dt.timezone.utc).strftime("%Y%m%dT%H%M%S_%fZ") + "_cpu_capacity")
            audit.mkdir(parents=True)
            with ExclusiveRunLock(scheduler.run_path / "cpu_capacity_handoff.lock"):
                write_json_once(audit / "request.json", {
                    "role": "cpu", "old_pid": args.old_pid,
                ...
                def guarded_stop(pid, run, runner):
                    ...
                    if outside:
                        ...
                        os.kill(pid, signal.SIGCONT)
                        raise SchedulerError(f"captured job outside CPU shard: {outside}")
