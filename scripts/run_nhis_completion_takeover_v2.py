"""Resource-only v2 handoff: preserve outputs across controller generations, up to 64 CPUs.
...
def check_artifact(path,job_id,manifest_seal,returncode,*,adopted):
    ...
    return {'status':'FC_ARTIFACT_READY_FOR_REVIEW' if valid else 'REQUIRES_REVIEW',
            ...
            'adopted_process':adopted,'output':str(path),'result_sha256':sha(result_path),
...
def run(root,control):
    ...
    if (handoff['manifest_sha256']!=manifest['manifest_sha256']
            ...
            or not Path(handoff['old_run']).is_absolute()):
    ...
    active={j:{'identity':i,'adopted':True,'output':Path(handoff['adopted_outputs'][j])}
            for j,i in handoff['adopted'].items()}
    for job, info in active.items():
        if info['output'].name!=job or not info['output'].resolve().is_relative_to(root.parent):
            raise ValueError('Invalid inherited output location')
    ...
    def save():
        atomic(control/'state.json',{'controller_pid':os.getpid(),'updated_unix':time.time(),
            ...
            'active':{j:{'pid':v['identity']['pid'],'start_ticks':v['identity']['start_ticks'],
                         'adopted':v['adopted'],'output':str(v['output'])}
    ...
    while pending or active:
        ...
        if type(maximum) is not int or not 1<=maximum<=64:raise ValueError('Invalid concurrency')
        while pending and len(active)<maximum:
            ...
            active[job]={'identity':identity,'adopted':False,'process':process,'log':log,'output':target}
