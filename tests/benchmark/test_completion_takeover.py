...
import run_nhis_completion_takeover as q
...
def test_actual_controller_adopts10_and_dispatches70_once(tmp_path,monkeypatch):
    ...
    q.atomic(control/'resources.json',{'concurrency':56})
    q.atomic(control/'handoff.json',{'manifest_sha256':'seal','new_controller_sha256':q.sha(q.__file__),
      'old_run':str(old),'old_controller':{'pid':999,'start_ticks':1},'completed':{},
      'adopted':{j:{'pid':i+1,'start_ticks':1} for i,j in enumerate(ids[:10])},'new_jobs':ids[10:]})
    def artifact(path,job):
        ...
        q.atomic(path/'result.json',{'status':'FC_COMPLETE_FEASIBLE','job_id':job,'manifest_sha256':'seal',
          'reload_exact':True,'formal_benchmark_admission':False,'S_T_evaluated':False,
          'completion':{'status':'COMPLETE_FEASIBLE'},'model_sha256':q.sha(model)})
    for job in ids[:10]:artifact(old/job,job)
    dispatched=[]
    monkeypatch.setattr(q,'process_alive',lambda info:info['pid']!=999 and len(dispatched)<46)
    ...
    q.run(root,control)
    assert len(dispatched)==len(set(dispatched))==70
    assert set(dispatched)==set(ids[10:])
    receipt=json.loads((control/'queue_receipt.json').read_text())
    assert receipt['jobs']==80 and not receipt['formal_benchmark_admission']
    assert all(v['status']=='FC_ARTIFACT_READY_FOR_REVIEW' for v in receipt['completed'].values())
    assert all(receipt['completed'][j]['exit_code'] is None for j in ids[:10])
